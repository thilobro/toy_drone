...
class Nmpc():
    ...
    def __init__(self, model, state_running_cost, state_terminal_cost,
        ...
        # self._ocp = self._build_nlp_ocp()
        self._ocp = self._build_sqp_ocp()
    ...
    def _build_sqp_ocp(self):
        ...
        for i in range(1, self._horizon):
            ...
            ocp_variables_at_lin += [ca.SX.sym(f'Vlin{i}', self._state_dim + self._controls_dim)]
        ...
        ocp_variables_at_lin += [ca.SX.sym(f'Vlin{self._horizon}', self._state_dim)]
        ...
        equality_constraints = ca.vertcat(*equality_constraints)
        ocp_variables = ca.vertcat(*ocp_variables)
        ocp_variables_at_lin = ca.vertcat(*ocp_variables_at_lin)
        # ocp_variables_at_lin = copy.deepcopy(ocp_variables)
        params += [ocp_variables_at_lin]
        equality_constraints_fun = ca.Function('equality_constraints', [ocp_variables],
                                               [equality_constraints])
        linearized_equality_constraints = equality_constraints_fun(ocp_variables_at_lin)\
            + ca.mtimes(ca.jacobian(equality_constraints, ocp_variables),
                        (ocp_variables - ocp_variables_at_lin))

        # build nlp
        qp = {'x': ocp_variables, 'f': cost_function,
              'g': linearized_equality_constraints, 'p': ca.vertcat(*params)}

        # build solver
        # solver_options = {'ipopt': {'print_level': 0}, 'print_time': False}
        self._solver = ca.qpsol('solver', 'qpoases', qp)

    def _build_nlp_ocp(self):
        """
        _build_nlp_ocp Build up OCP out of casadi expressions and generate numerical solver.
        """
        ocp_variables = []
        ocp_variables += [ca.SX.sym('V0', self._state_dim + self._controls_dim)]
        params = []
    ...
    def _solve_nlp_ocp(self, state, reference):
        """
        _solve_nlp_ocp Solve the OCP for a set of parameters.
        ...
        solution = self._solver(x0=self._V_initial, p=np.hstack((state, reference)), ubg=0, lbg=0,
                                ubx=ubx, lbx=lbx)
        return solution['x'].full().squeeze()

    def _solve_sqp_ocp(self, state, reference):
        """
        _solve_nlp_ocp Solve the OCP for a set of parameters.
        ...
        Returns:
            Optimal trajectory in state and controls
        """
        ocp_variables_at_lin = self._V_initial
        for i in range(100):
            lbx = [-ca.inf] * self._state_dim + [self._control_bounds[0]] * self._controls_dim
            lbx = lbx * (self._horizon) + [-ca.inf] * self._state_dim
            ubx = [ca.inf] * self._state_dim + [self._control_bounds[1]] * self._controls_dim
            ubx = ubx * (self._horizon) + [ca.inf] * self._state_dim
            solution = self._solver(x0=self._V_initial,
                                    p=np.hstack((state, reference, ocp_variables_at_lin)),
                                    ubg=0, lbg=0, ubx=ubx, lbx=lbx)
            if (ocp_variables_at_lin - solution['x'].full().squeeze()).all() < 1e-6:
                ocp_variables_at_lin = solution['x'].full().squeeze()
                break
            else:
                ocp_variables_at_lin = solution['x'].full().squeeze()

        return ocp_variables_at_lin
    ...
    def _extract_initial_guess(self, optimal_trajectory):
        ...
        return initial_guess.full().squeeze()
    ...
    def compute_control(self, state, reference):
        ...
        """
        # solve OCP
        optimal_trajectory = self._solve_sqp_ocp(state, reference)
